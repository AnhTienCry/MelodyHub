...
export function SongList({ isOpen, onClose, songs, currentSong, onSelect }: SongListProps) {
  return (
    <AnimatePresence>
      {isOpen && (
        <>
          ...
          >
            ...
            <div className="flex-1 overflow-y-auto no-scrollbar space-y-2">
              {songs.map((song, index) => (
                <motion.div
                  key={song.id ?? (song as unknown as { _id?: string })._id ?? `song-${index}`}
                ...
                >
                  <div className="relative w-12 h-12 rounded-lg overflow-hidden shrink-0">
                    <img src={song.cover || '/placeholder.svg'} alt={song.title} className="object-cover w-full h-full" />
                    {currentSong && currentSong.id === song.id && (
                  ...
                  <div className="flex-1 min-w-0">
                    <h3 className={`font-medium truncate ${currentSong && currentSong.id === song.id ? 'text-white' : 'text-white/80 group-hover:text-white'}`}>
                      {song.title}
                    </h3>
                    <p className="text-sm text-white/40 truncate group-hover:text-white/60">{song.artistId?.name}</p>
                  </div>

                  <span className="text-xs text-white/30 font-mono">{song.duration}</span>
                </motion.div>
              ))}
            </div>
          </motion.div>
...
// Default export kept for compatibility with older imports
